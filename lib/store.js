...
class Store extends EventEmitter {
  ...
  *scanJobsDir () {
  ...
  queue (name, priority) {
    ...
    if (!ret) {
      this.queues[key] = ret = new Queue(name, priority);
    }
    return ret;
  }
  ...
  *allCounters () {
    ...
    for (const queue of queues) {
      const { topic, priority, counter: { pending, running, finish } } = this.queues[queue];
      counters.push({
        topic, priority, pending, running, finish,
      });
    }
    return counters;
  }
  ...
  *appendJob (topic, priority, payload, payloadType = '', noRetry = false, jobId = '', createTime = 0) {
    ...
    if (this.ready) {
      ...
      this.emit('queue-update', jobStatus.queue);
    }
    return jobId;
  }
