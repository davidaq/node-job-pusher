...
class Store extends EventEmitter {
  ...
  *nextJob (topic) {
    for (let i = 0; i < 3; i++) {
      ...
      if (doc) {
        ...
        const [job] = cb => this.detail.findOne({ _id: doc._id });
        job.id = job._id;
        return job;
      }
    }
  ...
  *getJobs (where, page, pageSize = 20) {
    const [docs] = yield cb => this.detail.find(where).skip(pageSize * (page - 1)).limit(pageSize).exec(cb);
    return docs;
  }
