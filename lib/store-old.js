...
class Store extends EventEmitter {
  constructor () {
    ...
    this.topicStore = new Proxy({}, {
      get (target, key) {
        if (!target[key]) {
          target[key] = new TopicStore();
        }
        return target[key];
      }
    });
  ...
  *setup (persistDir) {
    ...
    this.queue = new DataStore({
      filename: path.join(persistDir, 'queue.db'),
      autoload: true,
    });
  ...
  *doneJob (job, isSuccess, noRetry = false) {
    const updateDoc = {};
    if (isSuccess) {
      updateDoc.expire = new Date(Date.now() + 1200 * 1000);
