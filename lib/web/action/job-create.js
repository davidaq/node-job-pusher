...
module.exports = function* (req, res) {
  if (req.method === 'POST') {
    ...
    for (let i = 0; i < repeat; i++) {
      const id = yield store.appendJob(job.topic, job.priority, job.payload, job.payloadType, !!job.noRetry);
      ids.push(id);
    }
    if (job.from === 'form') {
    ...
    const job = yield store.getJob(req.query.duplicate);
    if (job) {
      const id = yield store.appendJob(job.topic, job.priority, job.payload, job.payloadType);
